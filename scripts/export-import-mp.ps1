...
# Comment: 			PD 09-23-2015 - I take no credit for this script.  Leaving it here for various customizations I have made
#----------------------------------------------------------------------------- 
...
function updateManagedProperty {
    Param([Microsoft.Office.Server.Search.Administration.ManagedProperty] $newmp)
...
if ($import){
    if ((Get-ChildItem $logfile -ea SilentlyContinue) -eq $null){
        ...
        Import-Csv $logfile -Delimiter "," | %{
            ...
            $existingMp = Get-SPEnterpriseSearchMetadataManagedProperty -SearchApplication $ssa -Identity $mp.Name -ea SilentlyContinue;
            if ($existingMp -eq $null){
                try {
                ...
                Write-Host "Managed property $($mp.Name) already exists.. attempting merge of crawled properties" -ForegroundColor Yellow
                ac $importlog "$(Get-Date),$($mp.Name),[WRN],Managed Property,Managed property already exists"
                    
                updateManagedProperty $existingMp;
            }
        }
    }
}
